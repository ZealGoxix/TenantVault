...
const styles = StyleSheet.create({
  ...
  caseCell: { flex: 1 },
  caseLabel: { fontSize: 7, color: '#9CA3AF', textTransform: 'uppercase', letterSpacing: 1.5, marginBottom: 2 },
...
export function EvidenceReport({ caseData: c, evidence, auditLogs, generatedAt }: Props) {
  ...
  return (
    <Document title={`TenantVault Evidence Report — ${c.address}`}>
      <Page size="LETTER" style={styles.page}>
        ...
        {/* Audit log */}
        <Text style={styles.sectionTitle}>Complete Audit Log ({auditLogs.length} events)</Text>
        {auditLogs.map((log, i) => (
